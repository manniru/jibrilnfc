...
public class Dao {
	...
	public List lisTransact() {
		List<Transact> users = new ArrayList();		
		try {
        PreparedStatement ps = cn.prepareStatement("SELECT * FROM ADMIN.TRANSACT");
        ResultSet rs = ps.executeQuery();
        while (rs.next()) {
        	User user = new User();
	...
	public void addTransact(Transact tr) {
		try {
			PreparedStatement ps = cn.prepareStatement("INSERT INTO ADMIN.TRANSACT(MOBILENO, TAGID, DETAILS) VALUES(?,?,?)");
			ps.setString(1, tr.getMobileno());
			ps.setString(2, tr.getTagid());
			ps.setString(3, tr.getDetails());
			ps.executeUpdate();
